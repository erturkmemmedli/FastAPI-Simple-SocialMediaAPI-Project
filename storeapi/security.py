...
def access_token_expires_minute() -> int:
    return 30


def confirm_token_expires_minute() -> int:
    return 1440
...
def create_access_token(email: str):
    ...
    expire = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(
        minutes=access_token_expires_minute()
    )
...
def create_confirmation_token(email: str):
    ...
    expire = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(
        minutes=confirm_token_expires_minute()
    )
...
async def get_current_user(token: Annotated[str, Depends(oauth2_scheme)]):
    ...
    user = await get_user(email)
    if not user:
        raise create_credentials_exception("Could not find user for provided token")
    return user
